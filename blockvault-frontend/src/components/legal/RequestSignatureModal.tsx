  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-md flex items-start justify-center z-[999] p-4 py-24 overflow-y-auto">
      <Card variant="premium" className="w-full max-w-2xl shadow-2xl border-primary-500/30 animate-fade-in-up">
        <div className="p-8">
          {/* Modal Header */}
          <div className="flex justify-between items-start mb-8 pb-6 border-b border-secondary-700/30">
            <div>
              <h2 className="text-3xl font-bold text-white mb-2 text-gradient">Request Signatures</h2>
              <p className="text-sm text-text-secondary">Collect signatures from multiple parties with blockchain verification</p>
            </div>
            <Button variant="ghost" onClick={onClose} className="hover:bg-status-error/10 hover:text-status-error">
              <X className="w-5 h-5" />
            </Button>
          </div>
